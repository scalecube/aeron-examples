...
public class MdsRecordingWithLiveDestThroughput implements AutoCloseable {
  ...
  public static final String LIVE_CHANNEL =
      new ChannelUriStringBuilder()
          .media(UDP_MEDIA)
          .controlEndpoint(CONTROL_ENDPOINT)
          .endpoint("localhost:0")
  ...
  public static void main(final String[] args) throws InterruptedException {
    try (MdsRecordingWithLiveDestThroughput test = new MdsRecordingWithLiveDestThroughput()) {
  ...
  MdsRecordingWithLiveDestThroughput() {
  ...
  private static void awaitPublicationConnected(Publication publication) {
  ...
  private void streamMessagesForRecording() throws InterruptedException {
    try (Publication publication = aeron.addExclusivePublication(PUBLICATION_CHANNEL, STREAM_ID);
      ...
      subscription.addDestination(ChannelUri.addSessionId(LIVE_CHANNEL, publication.sessionId()));
      ...
      awaitSubscriptionConnected(idleStrategy, subscription);
      ...
      AgentRunner liveAgentRunner =
          new AgentRunner(NoOpIdleStrategy.INSTANCE, th -> {}, null, new LiveAgent(subscription));
      liveAgentRunner.run();
  ...
  private static void awaitSubscriptionConnected(
    ...
    System.out.println("### Await multi-destination subscription gets connected");
    while (!subscription.isConnected()) {
      idleStrategy.idle();
    }
    System.out.println("### Multi-destination subscription connected");
  }
  ...
  private static class LiveAgent implements Agent {
    ...
    public LiveAgent(Subscription subscription) {
    ...
    void onMessage(DirectBuffer buffer, int offset, int length, Header header) {
      ...
      totalLength += length + AeronHelper.HEADER_LENGTH;
    }
